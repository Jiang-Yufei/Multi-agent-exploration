...
void SDFMap::initMap(ros::NodeHandle& nh) {
  ...
  if (mp_->use_swarm_tf_) {
    for (auto& vert : mp_->vertices_) vert = mp_->rot_sw_ * vert + mp_->trans_sw_;
  }
...
shared_ptr<MapData> SDFMap::get_md(){
  

  if(md_ != nullptr){
    shared_ptr<MapData>md = std::make_shared<MapData>();
    md->occupancy_buffer_ = md_->occupancy_buffer_;
    md->occupancy_buffer_inflate_ = md_->occupancy_buffer_inflate_;
    md->distance_buffer_neg_ = md_->distance_buffer_neg_;
    md->distance_buffer_ = md_->distance_buffer_;
    md->tmp_buffer1_ = md_->tmp_buffer1_;
    md->tmp_buffer2_ = md_->tmp_buffer2_;

    md->count_hit_ = md_->count_hit_;
    md->count_miss_ = md_->count_miss_;
    md->count_hit_and_miss_ = md_->count_hit_and_miss_;

    md->flag_rayend_ = md_->flag_rayend_;
    md->flag_visited_ = md_->flag_visited_;

    md->raycast_num_ = md_->raycast_num_;
    md->cache_voxel_ = md_->cache_voxel_;
    md->local_bound_min_ = md_->local_bound_min_;
    md->local_bound_max_ = md_->local_bound_max_;
    md->update_min_ = md_->update_min_;
    md->update_max_ = md_->update_max_;
    md->all_min_ = md_->all_min_;
    md->all_max_ = md_->all_max_;

    md->reset_updated_box_ = md_->reset_updated_box_;

    md->swarm_transform_ = md_->swarm_transform_;
    return md;
  }
  return nullptr;
}

shared_ptr<MapParam> SDFMap::get_mp(){
  shared_ptr<MapParam> mp = std::make_shared<MapParam>();
  if(mp_ != nullptr){
    mp->map_origin_ = mp_->map_origin_;
    mp->map_size_ = mp_->map_size_;

    mp->map_min_boundary_ = mp_->map_min_boundary_;
    mp->map_max_boundary_ = mp_->map_max_boundary_;
    mp->map_voxel_num_ = mp_->map_voxel_num_;

    mp->resolution_ = mp_->resolution_;
    mp->resolution_inv_ = mp_->resolution_inv_;
    mp->obstacles_inflation_ = mp_->obstacles_inflation_;
    mp->virtual_ceil_height_ = mp_->virtual_ceil_height_;
    mp->ground_height_ = mp_->ground_height_;
    mp->box_min_ = mp_->box_min_;
    mp->box_max_ = mp_->box_max_;
    mp->box_mind_ = mp_->box_mind_;
    mp->box_maxd_ = mp_->box_maxd_;
    mp->default_dist_ = mp_->default_dist_;
    mp->optimistic_ = mp_->optimistic_;
    mp->signed_dist_ = mp_->signed_dist_;

    mp->p_hit_ = mp_->p_hit_;
    mp->p_miss_ = mp_->p_miss_;
    mp->p_min_ = mp_->p_min_;
    mp->p_max_ = mp_->p_max_;
    mp->p_occ_ = mp_->p_occ_;
    mp->prob_hit_log_ = mp_->prob_hit_log_;
    mp->prob_miss_log_ = mp_->prob_miss_log_;
    mp->clamp_min_log_ = mp_->clamp_min_log_;
    mp->clamp_max_log_ = mp_->clamp_max_log_;
    mp->min_occupancy_log_ = mp_->min_occupancy_log_;

    mp->max_ray_length_ = mp_->max_ray_length_;
    mp->local_bound_inflate_ = mp_->local_bound_inflate_;
    mp->local_map_margin_ = mp_->local_map_margin_;
    mp->unknown_flag_ = mp_->unknown_flag_;
    mp->no_drone_1_ = mp_->no_drone_1_;

    mp->use_swarm_tf_ = mp_->use_swarm_tf_;
    mp->rot_sw_ = mp_->rot_sw_;
    mp->trans_sw_ = mp_->trans_sw_;
    
    mp->vmin_ = mp_->vmin_;
    mp->vmax_ = mp_->vmax_;
    mp->vertices_ = mp_->vertices_;
    mp->normals_ = mp_->normals_;
    return mp;
  }
  return nullptr;
}
...
bool SDFMap::getBaseCoor(const int& id, Eigen::Vector4d& transform) {
  ...
  auto iter = md_->swarm_transform_.find(id);
  if (iter == md_->swarm_transform_.end()) {
