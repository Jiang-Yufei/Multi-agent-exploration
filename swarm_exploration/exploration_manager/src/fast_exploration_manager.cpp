...
int FastExplorationManager::updateFrontierStruct(const Eigen::Vector3d& pos) {
  ...
  vector<Vector3d> centers = frontier_finder_->getFrontierCenters();

  if (!radius.empty() && !centers.empty()){
    for (int i=0; i< radius.size(); i++){
      ...
      double sphere_radius = radius[i];  // 半径
      marker.pose.position.x = centers[i](0); // 球心坐标
