...
void FrontierFinder::searchFrontiers() {
  ...
  //for(int i=0; i<update_min.size(); i++){
  //  std::cout<<"update_min为"<<update_min(i);
  //}
...
double FrontierFinder::find_sample_Circles(Frontier& frontier){
  ...
  //get global map
  Vector3d pt;
  vector<Vector3d> obscales;
  Eigen::Vector3i min_idx, max_idx;

  shared_ptr<MapData> md_ = edt_env_->sdf_map_->get_md();
  shared_ptr<MapParam> mp_ = edt_env_->sdf_map_->get_mp();

  edt_env_->sdf_map_->posToIndex(md_->all_min_, min_idx);
  edt_env_->sdf_map_->posToIndex(md_->all_max_, max_idx);
  ...
  for (int x = min_idx[0]; x <= max_idx[0]; ++x)
    for (int y = min_idx[1]; y <= max_idx[1]; ++y)
      for (int z = min_idx[2]; z <= max_idx[2]; ++z) {
        if (md_->occupancy_buffer_[edt_env_->sdf_map_->toAddress(x, y, z)] >
            mp_->min_occupancy_log_) {
          Eigen::Vector3d pos;
          edt_env_->sdf_map_->indexToPos(Eigen::Vector3i(x, y, z), pos);
          if (pos(2) > 10) continue;
          if (pos(2) < -2) continue;
          pt[0] = pos(0);
          pt[1] = pos(1);
          pt[2] = pos(2);
          ...
          //std::cout << "pt:" << pt << endl;          
          obscales.push_back(pt);
        }
      }

  auto& center = frontier.average_;
  //std::cout << "center:"<< center << endl;

  //calculate nn point
  flann::Matrix<double> dataset(new double[obscales.size() * 3], obscales.size(), 3);
  for (size_t i = 0; i < obscales.size(); ++i) {
    dataset[i][0] = obscales[i](0);
    dataset[i][1] = obscales[i](1);
    dataset[i][2] = obscales[i](2);
  }
  flann::Index<flann::L2<double>> index(dataset, flann::KDTreeIndexParams(4));
  index.buildIndex();
  int num_neighbors = 1; // find 1 closest point
  flann::Matrix<int> indices(new int[num_neighbors], 1, num_neighbors);
  flann::Matrix<double> distances(new double[num_neighbors], 1, num_neighbors);
  double pt_data[3] = {center(0), center(1), center(2)};
  // Search using KD tree
  index.knnSearch(flann::Matrix<double>(&pt_data[0], 1, 3), indices, distances, num_neighbors, flann::SearchParams());

  // index and distance of nn
  int nearest_index = indices[0][0];
  double nearest_distance = distances[0][0];
  ros::Time sampleStart_t2 = ros::Time::now();
  
  //std::cout << "Nearest point index: " << nearest_index << std::endl;
  //std::cout << "Nearest point" << obscales[nearest_index] << std::endl;
  //std::cout << "Nearest point distance: " << nearest_distance << std::endl;
  //std::cout << "nn Search Time: " << sampleStart_t2 - sampleStart_t1 << std::endl;
...
vector<Vector3d> FrontierFinder::getFrontierCenters(){
  vector<Vector3d> frontiers;
  if((!tmp_frontiers_.empty())){
    for (auto frontier : tmp_frontiers_)
      frontiers.push_back(frontier.average_);
    return frontiers;
  }
...
void FrontierFinder::sampleViewpoints(Frontier& frontier) {
  ...
  for (double rc = candidate_rmin_, dr = (candidate_rmax_ - candidate_rmin_) / candidate_rnum_;
    ...
    //ros::Time sampleStart_t2 = ros::Time::now();
//    std::cout << "Sample时间为: " << sampleStart_t2-sampleStart_t1 << endl;
}
